...
class BlogTile extends StatefulWidget {
  ...
  const BlogTile({super.key, required this.blogId});
...
class _BlogTileState extends State<BlogTile> {
  ...
  Future<void> _loadBookmarkStatus() async {
    final bookmarks = await UserService().getUserBookmarks();
    setState(() {
      isBookmarked = bookmarks.contains(widget.blogId);
    });
  }
  ...
  void toggleBookmark() async {
    await BlogService().toggleBookmark(widget.blogId, isBookmarked);
    setState(() {
      isBookmarked = !isBookmarked;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<BlogModel?>(
      // To display the created blogs from the firestore
        future : BlogService().getBlogOnce(widget.blogId),
      ...
      builder: (context, snapshot) {
        ...
        DateTime createdAt = blog.timestamp;
        String formattedDate = DateFormat('MMM d, yyyy • hh:mm a').format(createdAt);
                            ...
                            onTap: () async {
                              ...
                              if(confirm == true) {
                                ...
                                await FirebaseFirestore.instance
                                    ...
                                    .doc(currentUserId)
                                    .update({
                                  'blogs': FieldValue.arrayRemove([blog.blogId])
                                });
                              }
                            },
                            child: Container(
